...
class TimeLoggerRolloverHandler(TimedRotatingFileHandler):
    ...
    def doRollover(self):
        ...
        dfn = f"{datetime.datetime.now().strftime('%Y%m%d')}.{log_type}.log"
...
log_error_file = path + '/' + '{}.error.log'.format(datetime.datetime.now().strftime('%Y%m%d'))
log_info_file = path + '/' + '{}.info.log'.format(datetime.datetime.now().strftime('%Y%m%d'))
...
print(os.popen("tasklist | findstr “python”").readlines())