...
from compare import gluList, gluInfo, patientTrendChart, patientPieChart, appReport,measure
        ...
        #
            ...
            if info['version'] == 2 or info['version'] == 1:
                ...
                if len(response['data']) == 0:
                    ...
                    DICT = {"type": "array", }
                    app_report = appReport.AppGluReport.get()
                    item = appReport.AppGluReport.item
                    DICT['items']=item
                    L = []
                    ...
                    for i in L:
                        ...
                        app_report["data"]["items"]["properties"][i] = DICT
        ...
        if read_yaml('headers')['Platform'] == '3':
            if info['version'] == 2 or info['version'] == 3:
                ...
                response = re.request_main('post', Url.GLU_WEB_MEASURE, headers=read_yaml('headers'),
                                           json=info['parame'])
                Encapsulation.repeatOne(Url.GLU_WEB_MEASURE, info, response, gluList.GluList.get())
